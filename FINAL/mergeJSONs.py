...
def merge_jsons(directory, json_list):
	with open("adj_list.json","a") as fp:
...
# generate list of json files in directory jsonTest
    #json_list = list_of_json_files_in_dir('jsonPR')

# merge all jaons in list into single master json in same dir as script
    #merge_jsons('jsonPR',json_list)
    
# clean master json list - output to new script
    cleanAdjList("reverse_index.json","cleaned_index.json")
