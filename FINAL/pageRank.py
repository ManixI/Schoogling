...
def pageRank(adj_list, d=0.85, e=0.0001):
	...
	ranked = pd.DataFrame(np.column_stack([URLlist, rankedList]), columns=['URL', 'Rank'])
	print(ranked)
	return ranked
...
def output_to_file(rankedDict):
  	with open("pageRanked", "w") as fp:
  		rankedDict.to_json(fp, orient='records', lines=True)

if __name__ == "__main__":

	# make sure you are getting a "cleaned" adjacency list
	adj_list = get_list_from_file("adj_list.json")
...
#	if pageRank == True:
#					if os.path.exists(os.path.abspath("pageRanked")):
#						file = open("pageRanked", 'r')
#						rankedDict = json.load(file)
#						sortedRanked = sorted(rankedDict.items(), key=operator.itemgetter(1), #reverse=True)
#						print(sortedRanked)
#						for y in sortedRanked:
#							for x in results:
#								if x['url'] == y[0]:
...
#									desc.append(x['desc'])
#					else:
#						print("Error, page ranked document not found")
